...
/**
 * Resolves the account-binding capability for one adapter type, or throws when
 * the type declares none. `getServerAdapter` is never null — it falls back to
 * a process adapter for an unknown type — so this always resolves *some*
 * module; the throw below fires only for a resolved module with no binding.
 *
 * `getServerAdapter` reads the same override-aware registry the account
 * picker's discovery route reads (`listServerAdapters()` in
 * `routes/adapter-accounts.ts`), so a plugin that registers under a built-in
 * type (for example `codex_local`) replaces the binding along with
 * everything else the registry serves for that type. A plugin override that
 * declares a valid displayed-code login capability but no account binding
 * hits this throw by design: without it, the promotion above would mint a
 * secret named for the built-in adapter's home variable over a credential
 * home nothing has run the built-in readiness check against. Discovery and
 * naming read the same map, so both stop working together instead of one
 * silently drifting ahead of the other.
 */
function requireAccountBinding(adapterType: string): AdapterAccountBinding {
...
export function accountSecretName(adapterType: string, handle: string): string {
  return `${requireAccountBinding(adapterType).secretPrefix}${handle}`;
}

/**
 * The human-readable description recorded on the account-home secret at
 * creation. Named from the same adapter-declared binding as the secret's own
 * name (its `envKey`, e.g. `CODEX_HOME` or `CLAUDE_CONFIG_DIR`), so the
 * description never assumes Codex for an adapter this promotion runs
 * generically.
 */
export function accountSecretDescription(adapterType: string, handle: string): string {
  return `${requireAccountBinding(adapterType).envKey} generated by logging into account ${handle}`;
}
            ...
            try {
              const createdSecret = await secretsSvc.create(
                context.companyId,
                {
                  ...
                  description: accountSecretDescription(context.adapterType, handle),
                },
